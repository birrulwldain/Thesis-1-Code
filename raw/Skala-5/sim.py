...
# Elemen target
BASE_ELEMENTS = ['Al', 'Ca', 'Fe', 'Si', 'Mg', 'Ti', 'Cr', 'Mn', 'Ni', 'Cu', 'Li','Ca', 'Mg', 'Na', 'K', 'O', 'H', 'N', 'Sr', 'Si', "C", 'Rb',"Co", "Pb", "Ba", "Zn", "Br", "V", "Zr", "Sn", "Ag", "Au"]
    ...
    def get_nist_data(self, element: str, sp_num: int) -> Tuple[List[List], float]:
        try:
            with pd.HDFStore(self.hdf_path, mode='r') as store:
                df = store.get('nist_lines')
                
                if df is not None:
                     # Defensive: Strip whitespace from columns
                     df.columns = df.columns.str.strip()
                     
                     with open("sim_debug.log", "a") as log:
                         log.write(f"Fetch {element} {sp_num}\n")
                         log.write(f"Cols Cleaned: {df.columns.tolist()}\n")

                if df is not None:
                     if 'sp_num' not in df.columns:
                          with open("sim_debug.log", "a") as log: log.write("CRITICAL: sp_num missing from columns!\n")
                          # Fallback or error?
                          if 'ion' in df.columns:
                               df.rename(columns={'ion': 'sp_num'}, inplace=True)
                     
                     if 'element' not in df.columns:
                          with open("sim_debug.log", "a") as log: log.write("CRITICAL: element missing from columns!\n")
...
class SpectrumSimulator:
    ...
    def simulate(self, atom_percentage: float = 1.0) -> Tuple[np.ndarray, np.ndarray, List[Tuple[float, float, str]]]:
        # Optimization: Create stick spectrum first, then convolve once.
        # This avoids generating a gaussian profile for every single line.
        
        # 1. Parse lines and calculate intensities
        lines_wl = []
        lines_int = []
        prominent_lines = []
        
        # Pre-calc partition function
        levels = {}
        # First pass for Z (could be cached if Z doesn't change often, but iterating list is fast enough)
        for data in self.nist_data:
            if not isinstance(data, (list, tuple)) or len(data) < 6: continue
            try:
        ...
        for data in self.nist_data:
            ...
            try:
                ...
                if all(v is not None for v in [wl, Aki, Ek, gk]):
                    ...
                    if not (self.wl_range[0] <= wl <= self.wl_range[1]):
                        continue
                        
                    intensity = self.calculate_intensity(Ek, gk, Aki, Z) * atom_percentage
                    lines_wl.append(wl)
                    lines_int.append(intensity)
                    
                    if intensity > 1e-3: # Store only meaningful lines for label
                        prominent_lines.append((wl, intensity, self.element_label))
            except: continue

        if not lines_wl:
            return self.wavelengths, np.zeros(self.resolution, dtype=np.float32), []

        # 2. Binning (Stick Spectrum)
        # Using histogram to bin intensities into the wavelength grid
        # self.wavelengths is linspace, so we can compute indices directly
        wl_min, wl_max = self.wl_range
        wl_step = (wl_max - wl_min) / (self.resolution - 1)
        
        indices = ((np.array(lines_wl) - wl_min) / wl_step).astype(int)
        valid_mask = (indices >= 0) & (indices < self.resolution)
        
        stick_spectrum = np.zeros(self.resolution, dtype=np.float32)
        np.add.at(stick_spectrum, indices[valid_mask], np.array(lines_int)[valid_mask])
        
        # 3. Convolution
        # Create Gaussian Kernel
        sigma_pixels = self.sigma / wl_step
        # Kernel size: +/- 4 sigma is usually enough (99.99%)
        k_radius = int(4 * sigma_pixels)
        k_size = 2 * k_radius + 1
        if k_size < 3: k_size = 3
        
        # x for kernel
        x_k = np.arange(-k_radius, k_radius + 1)
        kernel = np.exp(-0.5 * (x_k / sigma_pixels)**2)
        kernel /= (self.sigma * np.sqrt(2 * np.pi)) # Normalize amplitude-wise? 
        # Actually standard definition of intensity is integrated area. 
        # If stick spectrum height is 'intensity' (area under line), we want the gaussian area to be 1.
        # So sum(kernel) * pixel_width should be 1? Or sum(kernel) = 1?
        # Usually conv(stick, kernel) preserves sum if sum(kernel)=1.
        # Let's ensure sum(kernel) = 1 to conserve total energy.
        kernel /= kernel.sum() 
        
        # Perform convolution (using scipy or numpy)
        # Using numpy.convolve typically faster for small kernels than FFT
        # But we are in sim.py which imports torch. Let's use torch or scipy.signal?
        # Using simple np.convolve is fine.
        spectrum = np.convolve(stick_spectrum, kernel, mode='same')
        
        prominent_lines.sort(key=lambda x: x[1], reverse=True)
        return self.wavelengths, spectrum.astype(np.float32), prominent_lines[:20]

class MixedSpectrumSimulator:
    def __init__(self, simulators: List[SpectrumSimulator], electron_density: float, delta_E_max: Dict[str, float], ionization_energies: Dict[str, float], config: Dict = SIMULATION_CONFIG):
    ...
    def generate_spectrum(self, selected_elements: List[Tuple[str, float]], temperature: float) -> Tuple[np.ndarray, np.ndarray, Dict, List[Tuple[float, float, str]]]:
        ...
        for base_elem, percentage in selected_elements:
            ion_energy = self.ionization_energies.get(f"{base_elem} I", 0.0)
            if ion_energy == 0.0:
...
def run_simulation(selected_elements, temperature, electron_density, show_labels=True):
    ...
    nist_path = "nist_lines_all.h5"
    atomic_data_path = "atomic_data1.h5"
    ...
    # Pass global ionization_energies to the class
    mixed_simulator = MixedSpectrumSimulator(simulators, electron_density, delta_E_max_dict, ionization_energies)

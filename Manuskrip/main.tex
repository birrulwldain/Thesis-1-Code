...
Menghadapi kebuntuan tiga arah ini, perancangan arsitektur \textit{Physics-Informed Surrogate Model} tidak lagi berstatus sebagai opsi, melainkan keharusan mutlak. Arsitektur hibrida ini berdiri sebagai satu-satunya entitas yang selamat: integritas hukum mekanika kuantum diamankan melalui generator \textit{partial}-LTE (pLTE) dan RTE \textit{offline} yang memproduksi anomali spektral deterministik melalui model dua-zona komprehensif \cite{zaitsev_two-zone_2024}, sementara hambatan komputasi dielakkan dengan merekam kausalitas tersebut ke dalam algoritma \textit{Support Vector Regression} (SVR) \textit{online}. Metodologi hibrida ini memastikan diagnostik termodinamika tereksekusi secara instan tanpa pernah mengkhianati fondasi fisika fundamentalnya \cite{favre_merlin_2025}.

% ============================================================================
\section{Metode Komputasi: Arsitektur \textit{Physics-Informed Surrogate Model}}
% ============================================================================

Desain metodologis dalam penelitian ini dikonstruksi berdasarkan arsitektur hibrida fase-ganda (\textit{dual-phase hybrid architecture}), yang secara ketat memisahkan proses pembangkitan data fisis (\textit{offline forward modeling}) dari proses ekstraksi parameter (\textit{online inverse modeling}). Pemisahan algoritma ini merupakan taktik komputasional esensial untuk memintas hambatan komputasi (\textit{computational bottleneck}) pada penyelesaian matriks Persamaan Transfer Radiatif (RTE), sekaligus mengeliminasi sifat stokastik \textit{black-box} dari \textit{machine learning} konvensional. Melalui kerangka kerja ini, komputasi pencarian akar (\textit{root-finding}) tidak dieksekusi secara iteratif pada spektrum eksperimen riil, melainkan disubstitusi oleh model proksi (\textit{Surrogate Model}) yang telah diinformasikan oleh hukum fisika fundamental.

Fase pertama, yakni Pembangkitan Data Deterministik (\textit{Offline Data Generation}), difungsikan murni sebagai mesin fisika yang memproduksi semesta data spektral sintetis (\textit{synthetic manifold}). Generator analitik ini merajut Kinetika Termodinamika pLTE (evaluasi Persamaan Saha dan Distribusi Boltzmann secara terpisah) dan Mekanika Spektral (pelebaran Stark dan Doppler berbasis profil Voigt) ke dalam model plasma Dua-Zona spasial. Keluaran dari fase ini bukanlah spektrum ideal transparan, melainkan ribuan profil spektral yang telah secara fisis terdistorsi oleh redaman opasitas optik melalui penyelesaian analitik RTE. Hasilnya, seluruh dataset yang terbangun telah terkunci secara natif di dalam ruang metrik yang mematuhi hukum termodinamika absolut (\textit{pre-constrained physical manifold}).

Fase kedua memformulasikan \textit{Support Vector Regression} (SVR) murni sebagai instrumen \textit{Surrogate Model} untuk memecahkan inversi diagnostik secara seketika (\textit{real-time online inversion}). Berbeda dengan arsitektur optimasi \textit{multi-constraint} iteratif (seperti pendekatan regresi klasik) yang memaksakan validasi batasan Saha-Boltzmann secara bersamaan saat pencarian akar berjalan, SVR pada penelitian ini beroperasi pada paradigma \textit{Pre-Constrained Multi-Constraint}. Karena model SVR dilatih secara eksklusif menggunakan semesta data sintetis dari fase pertama, ruang regresi matematisnya secara otomatis mewarisi kausalitas mekanika kuantum dan redaman \textit{self-absorption}. Algoritma beroperasi bukan sebagai pencocok pola statistik buta, melainkan mengeksekusi pemetaan regresif instan antara morfologi cekungan Fraunhofer eksperimental terhadap parameter suhu inti ($T_e$) dan densitas ($n_e$).

Sinergi algoritmik dari mekanisme fase-ganda ini memastikan bahwa ekstraksi parameter dari data laboratorium dapat dieksekusi tanpa terjebak dalam paradoks melingkar opasitas optik. Ketika spektrum LIBS eksperimental yang terdistorsi oleh \textit{self-absorption} diumpankan ke dalam sistem, \textit{Surrogate Model} secara instan mengekstraksi parameter termodinamika tanpa memerlukan satu pun iterasi kalkulasi integrasi numerik RTE lanjutan. Metodologi hibrida ini memberikan resolusi arsitektural mutlak: menjaga kepatuhan fundamental terhadap integritas fisis tingkat dasar, sekaligus mencapai kelayakan waktu operasional (\textit{computational tractability}) tingkat tinggi yang mustahil direalisasikan oleh paradigma analitik murni.
...
\subsection{Kepatuhan Sokratik pada Parameter Fundamental}

Dalam mengorkestrasi fase pembangkitan data, model secara komprehensif menyapu ruang dimensi parameter ($T_e \in [4000, 20000]$\,K dan $n_e \in [10^{14}, 10^{18}]$\,cm$^{-3}$). Pada tahap ini, penelitian ini menerapkan ketidaktahuan Sokratik (\textit{Socratic ignorance}) absolut terhadap seluruh input atomik makroskopis: probabilitas transisi $A_{ki}$ dan berat statistik $g_i$ hanya bersumber secara murni dari Basis Data Spektra Atom NIST. Penampang lintang kolisional tingkat energi intermediet yang absen secara eksplisit tidak diaproksimasi menggunakan korelasi matematis buatan. Kedisiplinan penolakan asumsi parameter semu (\textit{pseudo-parameters}) ini mengeliminasi ambiguitas, memastikan bahwa proksi regresi instan (\textit{Surrogate Model}) dibangun murni di atas fondasi integritas fisis mekanika kuantum \cite{favre_towards_2025,lin_enhancing_2026}.

...
% Penomoran seksi diaktifkan standar
...
Prosedur pelaksanaan penelitian ini terdiri dari 4 (empat) tahapan utama sebagaimana diilustrasikan dalam diagram alir pada Gambar~\ref{fig:diagram-alir}. Penelitian mengikuti alur kerja yang mengintegrasikan pendekatan berbasis simulasi fisika plasma dengan teknik \textit{deep learning}, di mana keterbatasan jumlah data eksperimental (24 sampel) diatasi dengan memanfaatkan pengetahuan fisika plasma melalui data sintetis \cite{wang2024, favre2025b} sebelum mentransfer representasi yang telah dipelajari ke domain eksperimental.
...
\end{table}
    ...
    \item \textbf{Penghitungan populasi analit} memanfaatkan fondasi kesetimbangan termodinamika lokal (\textit{Local Thermodynamic Equilibrium} / LTE). Distribusi probabilitas elektron menempati keadaan tereksitasi dihitung berdasarkan Distribusi Boltzmann \cite{fujimoto2004}:
    \begin{equation}
        n_{k,z} = n_z \frac{g_{k,z}}{U_z(T_e)} \exp\left(-\frac{E_{k,z}}{k_B T_e}\right)
        \label{eq:boltzmann}
    \end{equation}
    di mana $n_{k,z}$ adalah populasi spesies pada tingkat energi $k$ dan status ionisasi $z$, $g_{k,z}$ adalah degenerasi statistik dari tingkat energi eksitasi $E_{k,z}$, $U_z(T_e)$ merupakan fungsi partisi, dan $k_B$ melambangkan konstanta Boltzmann. Adapun keseimbangan jumlah partikel antar tingkat ionisasi yang berurutan (netral vs ion) diseimbangkan melalui Persamaan Saha:
    \begin{equation}
        ...
        \label{eq:saha}
    \end{equation}
    dengan $n_e$ merepresentasikan densitas elektron, $m_e$ massa elektron, $E_{\infty,z}$ energi ionisasi, dan $\Delta E_{\infty,z}$ koreksi penurunan batas ionisasi akibat rapatnya interaksi plasma. Parameter plasma dominan pada persamaan di atas selanjutnya di-sampling secara acak seragam untuk menghasilkan variasi dataset: $T_e \in [6.000\text{--}15.000]$ K, $n_e \in [10^{16}\text{--}10^{17}]$ cm$^{-3}$. Rentang ini dipilih berdasarkan kondisi tipikal fluktuasi plasma LIBS pada tekanan atmosfer \cite{cristoforetti2010}.
    ...
    \item Ringkasan seluruh parameter fisis dan komputasional yang dilibatkan dalam simulasi \textit{forward model} spektral sintetis ini dirangkum pada Tabel~\ref{tab:parameter-simulasi}.
\end{enumerate}
...
Total Spektrum Sintetis & $10.000$ pasangan set & Varians komprehensif matriks untuk proses \textit{Deep Learning} \\
\bottomrule
\end{tabular}
\end{table}

...
Laser-Induced Breakdown Spectroscopy (LIBS) merupakan teknik spektroskopi emisi yang telah banyak digunakan untuk analisis komposisi unsur pada berbagai material kompleks, termasuk mineral, batuan, dan material tanah \cite{legnaioli2025}. Kemampuan LIBS untuk melakukan analisis multi-elemen secara cepat tanpa memerlukan preparasi sampel yang kompleks menjadikannya metode yang menarik untuk aplikasi geokimia dan eksplorasi sumber daya alam \cite{sawyers2025}. Berbagai penelitian menunjukkan bahwa LIBS telah berhasil digunakan untuk kuantifikasi unsur pada sampel geologi melalui beragam pendekatan analitik, mulai dari regresi multivariat, kurva kalibrasi univariat berbasis garis spektral tertentu \cite{zhang2025}, hingga metode berbasis \textit{machine learning} \cite{babos2024, manzoor2025}. Pendekatan-pendekatan tersebut mampu mencapai akurasi analitik yang tinggi, namun keberhasilan kuantifikasi sangat bergantung pada strategi pemodelan spektrum serta sensitivitas metode terhadap efek matriks pada sampel geologi yang kompleks \cite{hao2024, manelski2024}. Kondisi ini menjadi semakin relevan pada analisis tanah vulkanik, yang secara inheren memiliki komposisi multi-elemen dengan variasi matriks yang tinggi.

Dalam pendekatan kuantitatif LIBS, salah satu metode yang banyak digunakan adalah \textit{Calibration-Free LIBS} (CF-LIBS), yang memungkinkan estimasi komposisi unsur tanpa memerlukan sampel standar kalibrasi. Metode ini bergantung pada ekstraksi parameter plasma fundamental seperti temperatur elektron ($T_e$) dan densitas elektron ($n_e$), yang dihitung berdasarkan asumsi \textit{Local Thermodynamic Equilibrium} (LTE) \cite{cristoforetti2010}. Namun, asumsi plasma homogen dalam kondisi LTE sering kali tidak memadai untuk mendeskripsikan plasma LIBS yang bersifat sangat transien dan bergradien tinggi \cite{zaitsev2024, bultel2025}. Penyimpangan dari kondisi LTE ini dapat memicu distorsi spektral akibat efek opasitas optik dan penyerapan mandiri (\textit{self-absorption}), yang pada akhirnya mengganggu stabilitas estimasi temperatur dan densitas plasma \cite{tang2024, hansen2021}. Pendekatan berbasis model fisika plasma seperti model \textit{radiative transfer} adaptif MERLIN \cite{favre2025a} yang memanfaatkan distribusi Saha–Boltzmann serta Persamaan Transfer Radiatif menunjukkan bahwa rekonstruksi spektrum LIBS yang kompleks dapat dicapai dengan mempertahankan konsistensi termodinamika plasma, meskipun implementasinya masih menghadapi hambatan komputasi yang signifikan akibat kebutuhan inversi numerik nonlinier \cite{favre2025b}.

Perkembangan metode \textit{machine learning} dan \textit{deep learning} telah menawarkan alternatif untuk mempercepat analisis spektrum LIBS dengan mempelajari hubungan nonlinier antara spektrum dan parameter plasma secara langsung dari data. \textit{Convolutional Neural Networks} (CNN) mampu menangkap fitur lokal seperti profil garis emisi, sementara arsitektur Transformer memodelkan dependensi jarak jauh sepanjang sumbu panjang gelombang \cite{liu2026}. Model hibrida CNN–Transformer telah menunjukkan hasil menjanjikan untuk kuantifikasi unsur jejak pada baja \cite{liu2026} dan untuk klasifikasi signatur spektral pada sampel multi-elemen \cite{walidain2026}. Namun, sebagian besar model \textit{data-driven} beroperasi sebagai \textit{black box} yang tidak secara eksplisit mempertimbangkan hukum fisika plasma yang mendasari pembentukan spektrum emisi \cite{hao2024}. Fakta fisik fundamental yang belum dieksploitasi adalah: spektrum LIBS poliatomik merupakan superposisi terbobot konsentrasi dari spektrum monoatomik penyusunnya. Seluruh model eksisting—baik berbasis fisika maupun \textit{data-driven}—memperlakukan spektrum sebagai \textit{single signal} dan mengabaikan struktur komposisional ini. \textbf{Belum ada penelitian sebelumnya yang secara eksplisit memodelkan dekomposisi spektrum campuran ke dalam konstituennya dalam kerangka pembelajaran.} \textit{Cross-attention}, mekanisme yang awalnya diusulkan untuk translasi sekuens-ke-sekuens \cite{vaswani2017}, sangat ideal untuk tugas ini: decoder dapat meng-\textit{query} representasi elemen dari encoder untuk mengetahui kontribusi monoatomik mana yang membentuk observasi poliatomik tertentu \cite{liu2026, walidain2026}.
...
\subsection{Kebaruan (\textit{Novelty})}
\begin{enumerate}[label=\arabic*.]
...
\section{Peta Jalan (\textit{Roadmap}) Penelitian}
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{peta-jalan.pdf}
    \caption{Peta jalan penelitian menunjukkan progresi dari fondasi riset grup menuju riset AI spektral LIBS mutakhir.}
    \label{fig:roadmap}
\end{figure}

\input{tabel-roadmap.tex}
...
\textbf{NIST Atomic Spectra Database (ASD)}: Merupakan basis kompilasi standar yang diakses langsung pada server \textit{National Institute of Standards and Technology} \cite{kramida2024}. Ekstraksi dari tabel spektra ini menyupli pondasi variabel kuantum seperti batas energi ionisasi ($E_{\infty,i}$), eksitasi energi status ($E_k, E_i$), degenerasi probabilitas ($g_k, g_i$), hingga peluruhan transisi foton ($A_{ki}$) yang menentukan keberhasilan dari algoritma fisika plasma termodinamika buatan kita.


\subsection{Diagram Alir Penelitian}

Secara komprehensif, kerangka kerja eksperimental dan komputasional dalam penelitian ini divisualisasikan melalui diagram alir pada Gambar~\ref{fig:diagram-alir}.
...
\midrule
Rentang Suhu Elektron ($T_e$) & $6.000 - 15.000$ K & Memodelkan fluktuasi pendinginan plasma \cite{cristoforetti2010} \\
...
\begin{enumerate}[label=\arabic*., leftmargin=2cm]
    \item \textbf{Pengumpulan data transisi atomik} diunduh secara algoritmik dari \textit{database} referensi \cite{kramida2024}. Parameter konstan panjang gelombang ($\lambda_{ki}$), Einstein ($A_{ki}$), dan termal nukleus ($E_i$, $g_i$) diseleksi khusus menurut Tabel~\ref{tab:komposisi-tanah}.
    \item \textbf{Penghitungan populasi analit} bersandar penuh pada kesetimbangan termodinamika lokal / LTE. Kalkulasi fraksi status terspesifikasi mengikuti perumusan Distribusi probabilitas keadaan tereksitasi Boltzmann \cite{fujimoto2004}:
    \begin{equation}
...
\begin{itemize}[leftmargin=2cm]
    ...
    \item \textbf{4 model baseline} sebagai komparasi ketangguhan: PLS—\textit{Partial Least Squares} \cite{wold2001} sebagai representasi perbandingan regresi kemometrik klasik, model spasial diskrit (CNN-only), penyesuai sekuens independen (Transformer-only), dan arsitektur canggih \textit{Informer} \cite{zhou2021, walidain2026}.
...
\bibitem{legnaioli2025}
Legnaioli, S. et al. (2025). ``Laser-Induced Breakdown Spectroscopy Analysis of Lithium: A Comprehensive Review. \textit{Sensors}, 25(24), 7689.

\bibitem{sawyers2025}
Sawyers, E.R. et al. (2025). ``Database development and LIBS calibration for the LIBS-Raman Sensor for planetary exploration. \textit{Icarus}, 442, 116742.

\bibitem{zhang2025}
Zhang, Y., Yang, G., Wang, B.-X., \& Zhang, X. (2025). ``Improving the accuracy of laser-induced breakdown spectroscopy using the single-element standard curve calibration. \textit{Instrumentation Science \& Technology}, 1-20.

\bibitem{babos2024}
Babos, D.V. et al. (2024). ``Laser-induced breakdown spectroscopy as an analytical tool for total carbon quantification in tropical and subtropical soils. \textit{Frontiers in Soil Science}, 3, 1242647.

\bibitem{manzoor2025}
Manzoor, M. et al. (2025). ``A machine learning assisted approach to classify rose species and varieties with laser induced breakdown spectroscopy. \textit{Analytica Chimica Acta}, 1373, 344489.

\bibitem{hao2024}
Hao, Z. et al. (2024). ``Machine learning in laser-induced breakdown spectroscopy: A review. \textit{Frontiers of Physics}, 19(6), 62501.

\bibitem{manelski2024}
Manelski, H.T. et al. (2024). ``LIBS plasma diagnostics with SuperCam on Mars: Implications for quantification of elemental abundances. \textit{Spectrochimica Acta Part B}, 222, 107061.

\bibitem{cristoforetti2010}
Cristoforetti, G., De Giacomo, A., Dell'Aglio, M., et al. (2010). ``Local Thermodynamic Equilibrium in Laser-Induced Breakdown Spectroscopy: Beyond the McWhirter criterion.'' \textit{Spectrochimica Acta Part B: Atomic Spectroscopy}, 65(1), 86-95.

\bibitem{zaitsev2024}
Zaitsev, S.M. et al. (2024). ``Two-Zone Model of Laser-Induced Plasma. \textit{Journal of Applied Spectroscopy}, 90(6), 1183-1189.

\bibitem{bultel2025}
Bultel, A., Morel, V., \& Favre, A. (2025). ``The McWhirter Criterion Revisited for Laser-Induced Plasmas. \textit{ICPIG International Conference}.

\bibitem{tang2024}
Tang, Y. \& Zhao, N. (2024). ``A Review of Development in the Research of Self-Absorption on Laser-Induced Breakdown Spectroscopy. \textit{Atomic Spectroscopy}, 45(04), 336-357.

\bibitem{hansen2021}
Hansen, P.B. et al. (2021). ``Modeling of time-resolved LIBS spectra obtained in Martian atmospheric conditions with a stationary plasma approach. \textit{Spectrochimica Acta Part B}, 178, 106115.

\bibitem{favre2025a}
Favre, A. et al. (2025a). ``MERLIN, an adaptative LTE radiative transfer model for any mixture. \textit{J. Quant. Spectrosc. Radiat. Transfer}, 330, 109222.

\bibitem{favre2025b}
Favre, M., et al. (2025). ``Synthetic spectral generation and domain adaptation for robust calibration-free LIBS.'' \textit{Spectrochimica Acta Part B: Atomic Spectroscopy}, 211, 106820.

\bibitem{liu2026}
Liu, Y., et al. (2026). ``Transformer-based cross-attention mechanisms for multi-channel spectral decomposition.'' \textit{IEEE Transactions on Pattern Analysis and Machine Intelligence}.

\bibitem{walidain2026}
Walidain, B., et al. (2026). ``Classification of soil mineralogy via Laser-Induced Breakdown Spectroscopy and Informer time-series architecture.'' \textit{IOP Machine Learning: Science and Technology}.
...
\bibitem{mitaphonna2021}
Mitaphonna, R., et al. (2021). ``Preliminary evaluation of chemical component in the 2004 indian ocean giant tsunami impacted soil using a Co2 Laser Induced Breakdown Spectroscopy (LIBS). \textit{Journal of Physics: Conference Series}, 1816(1), 012035.

\bibitem{mitaphonna_eval_2023}
Mitaphonna, R., et al. (2023). ``Evaluation of Geochemical Signature in Soil Sampled from a 2004 Indian Ocean Tsunami-stricken Region... Using SEM-EDX Spectroscopy. \textit{Philippine Journal of Science}, 152(1).

\bibitem{ramli2023}
Ramli, M., et al. (2023). ``Elemental analysis of tsunami soil collected from Aceh of Indonesia using scanning electron microscope-energy dispersive x-ray (SEM-EDX). \textit{AIP Conference Proceedings}, 020011.

\bibitem{mitaphonna_xrf_2023}
Mitaphonna, R., et al. (2023). ``X-Ray fluorescence (XRF) investigation of soil sample from 2004 Indian Ocean tsunami affected region in Aceh of Indonesia. \textit{AIP Conference Proceedings}, 050001.

\bibitem{mitaphonna2023}
Mitaphonna, R., Ramli, M., Ismail, N., Hartadi, B. S., \& Idris, N. (2023). ``Identification of possible preserved 2004 Indian Ocean tsunami deposits collected from Pulot Village in Aceh Besar Regency, Indonesia.'' \textit{Journal of Physics: Conference Series}, 2582(1), 012033.

\bibitem{mitaphonna_prelim_2024}
Mitaphonna, R., et al. (2024). ``Preliminary identification of candidate layer of 2004 Indian Ocean tsunami sediment deposited in Seungko Meulat village... \textit{AIP Conference Proceedings}, 040012.

\bibitem{mitaphonna_char_2023}
Mitaphonna, R., et al. (2023). ``Characteristics of plasma produced from tsunami impacted soil sample using a transversely excited atmospheric pressure-carbon dioxide (TEA-CO2) laser. \textit{AIP Conference Proceedings}, 020010.

\bibitem{mitaphonna_pulsed_2024}
Mitaphonna, R., et al. (2024). ``A pulsed carbon dioxide laser-induced breakdown analysis for chemical profile of tsunami-affected soil. \textit{Global Journal of Environmental Science and Management}, 10(3).

\bibitem{mitaphonna2024}
Mitaphonna, R., Ramli, M., Ismail, N., \& Idris, N. (2024). ``Qualitative Geochemical Analysis of the 2004 Indian Ocean Giant Tsunami Deposits Excavated at Seungko Mulat Located in Aceh Besar of Indonesia Using Laser-Induced Breakdown Spectroscopy.'' \textit{Indonesian Journal of Chemistry}, 24(3).

\bibitem{khumaeni2025}
Khumaeni, A., et al. (2025). ``Geochemical profiling of volcanic soils from Mount Merapi using Nd:YAG laser-induced breakdown spectroscopy.'' \textit{Analytical Chemistry Research}, 12, 100-110.

\bibitem{wang2024}
Wang, Z., et al. (2024). ``Physics-informed neural networks for qualitative and quantitative analysis of laser-induced breakdown spectroscopy.'' \textit{Journal of Analytical Atomic Spectrometry}, 39(2), 245-256.
...
\bibitem{kramida2024}
Kramida, A., Ralchenko, Yu., Reader, J., and NIST ASD Team (2024). \textit{NIST Atomic Spectra Database (ver. 5.11)}. National Institute of Standards and Technology, Gaithersburg, MD. Tersedia daring: \url{https://physics.nist.gov/asd}.

\bibitem{fujimoto2004}
Fujimoto, T. (2004). \textit{Plasma Spectroscopy}. Oxford Clarendon Press.

\bibitem{griem1974}
Griem, H. R. (1974). \textit{Spectral Line Broadening by Plasmas}. Academic Press, New York.

\bibitem{kingma2015}
Kingma, D. P., \& Ba, J. (2015). ``Adam: A method for stochastic optimization.'' \textit{International Conference on Learning Representations (ICLR)}.

\bibitem{loshchilov2017}
Loshchilov, I., \& Hutter, F. (2017). ``SGDR: Stochastic gradient descent with warm restarts.'' \textit{International Conference on Learning Representations (ICLR)}.

\bibitem{ganin2016}
Ganin, Y., Ustinova, E., Ajakan, H., Germain, P., Larochelle, H., Laviolette, F., Marchand, M., \& Lempitsky, V. (2016). ``Domain-adversarial training of neural networks.'' \textit{The Journal of Machine Learning Research}, 17(1), 2096-2030.

\bibitem{wold2001}
Wold, S., Sjöström, M., \& Eriksson, L. (2001). ``PLS-regression: a basic tool of chemometrics.'' \textit{Chemometrics and Intelligent Laboratory Systems}, 58(2), 109-130.
...
\bibitem{ioffe2015}
Ioffe, S., \& Szegedy, C. (2015). ``Batch normalization: Accelerating deep network training by reducing internal covariate shift.'' \textit{International Conference on Machine Learning (ICML)}.
